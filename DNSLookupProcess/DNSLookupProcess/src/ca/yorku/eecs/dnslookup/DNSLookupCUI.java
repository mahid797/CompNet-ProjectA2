...
public class DNSLookupCUI implements DNSProcessListener {

    private static final int MAX_INDIRECTION_LEVEL = 10;

    private static boolean verboseTracing = false;
    private static DNSLookupProcess lookupService;
    ...
    public static void main(String[] args) {
        ...
        try {
            lookupService = new DNSLookupProcess(argNameServer, new DNSLookupCUI());
     ...
     */
    public void beforeSendingQuery(DNSQuestion question, InetAddress server, int transactionID) {
        if (verboseTracing)
            System.out.printf("\n\nQuery ID     %d %s  %s --> %s\n",
    ...
    public void receivedResponse(int receivedTransactionId, boolean authoritative, int error) {
        if (verboseTracing)
            System.out.printf("Response ID: %d Authoritative = %b Error = %x (%s)\n",
                    receivedTransactionId & 0xFFFF, authoritative, error, dnsErrorMessage(error));
    }
     ...
     */
    public void beforeProcessingAnswerSection(int num_answers) {
        if (verboseTracing)
     ...
     */
    private static void printResults(DNSQuestion question, Collection<ResourceRecord> results) {
        if (results.isEmpty())
